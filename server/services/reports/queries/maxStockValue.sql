...
select top 1 min(productMovementByDay.date) as date, productMovementByDay.value from (
    select  
    ...
    group by productMovement.calendarDate ) productMovementByDay
group by productMovementByDay.value
order by productMovementByDay.value desc

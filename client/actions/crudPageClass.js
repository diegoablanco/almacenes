...
export class CrudPageActions{
    constructor (crudPage, serviceActions, selectors){
        this.serviceActions = serviceActions
        this.selectors = selectors
        this.actionTypes = getActionTypes(crudPage)
    }
    ...
    loadGrid(){
        return (dispatch, getState) => {
            const {filter} = selectors.getUiState(getState())
            dispatch(serviceActions.find(filter)).then(result => {
                dispatch({ type: actionTypes.BUILD_ROWS, result: result.value })
            })
        }
    }        
    sortGrid(sortingColumns){
        return (dispatch, getState) => {
                ...
                dispatch({type: actionTypes.RESET_PAGE_NUMBER})
                const {filter} = selectors.getUiState(getState())
                dispatch(serviceActions.find(filter)).then(result => {
                    dispatch({ type: actionTypes.BUILD_ROWS, result: result.value })
                })
